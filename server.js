...
io.on("connection", (socket) => {
  ...
  socket.on("join", ({ name, room, isHost: host }) => {
    ...
    io.to(room).emit("timerSet", { timerDuration: rooms[room].timerDuration });
  });
  ...
  // TIMER EVENTS
  socket.on("timerSet", ({ room, timerDuration }) => {
    if (!rooms[room]) return;
    rooms[room].timerDuration = timerDuration;
    rooms[room].timerLeft = timerDuration;
    rooms[room].timerActive = false;
    io.to(room).emit("timerSet", { timerDuration });
  });
  socket.on("timerStart", ({ room }) => {
    if (!rooms[room]) return;
    rooms[room].timerActive = true;
    io.to(room).emit("timerStart");
    // Option: global interval
    if (rooms[room].timerInterval) clearInterval(rooms[room].timerInterval);
    rooms[room].timerInterval = setInterval(() => {
      if (rooms[room] && rooms[room].timerActive && rooms[room].timerLeft > 0) {
        rooms[room].timerLeft--;
        io.to(room).emit("timerSet", { timerDuration: rooms[room].timerLeft });
        if (rooms[room].timerLeft <= 0) {
          rooms[room].timerActive = false;
  ...
  socket.on("timerReset", ({ room }) => {
    if (!rooms[room]) return;
    rooms[room].timerLeft = rooms[room].timerDuration;
    rooms[room].timerActive = false;
    if (rooms[room].timerInterval) clearInterval(rooms[room].timerInterval);
